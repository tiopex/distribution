...
RUN apt-get update --fix-missing\
 ...
 && apt-get install -y locales sudo curl xz-utils ca-certificates
...
RUN apt-get install -y \
    ...
    xmlstarlet rsync \
    libdrm-dev libxcb-present-dev libxcb-dri2-0-dev libxcb-dri3-dev libxcb-glx0-dev libxcb-shm0-dev \
    libxshmfence-dev libgl1-mesa-dev libegl1-mesa-dev libx11-dev libxext-dev libwayland-dev libasound2-dev \
    libx11-dev libxrandr-dev libxext-dev libx11-xcb-dev

# Cross compile ARM → x86 fallback
RUN if [ "$(uname -m)" = "aarch64" ]; then \
       apt-get install -y --no-install-recommends qemu-user-binfmt libc6-dev-amd64-cross; \
    fi
